 ...
 * Returns      : 1 || 0
 * Purpose      : determines if char is lower case and is an asterisks
 */
int isLowerCase(char *inputString)
{
    
    if ((*inputString >= 'a' && *inputString <= 'z') || *inputString == '*') {
            return 1; // if True
        }
    return 0;
}
        ...
        do
        {
            while(fgets(buff, MAX_COMMAND_LEN, fp) != NULL)
            {
                ...
                else
                {
                    ...
                    //Gets the first element on the line (commandName) - if invalid call error function.
                    pch = strtok(buff," \t");
                    if(pch != NULL)
                    {
                    ...
                    //Gets the second element on the line (task duration in minutes)
                    pch = strtok(NULL, " \t");
                    digCheck = strtol(pch, &ptr, 10);
                    if(digCheck != 0 || errno != EINVAL) //Ensures that there was only a number in this field no other characters.
                    {
                        strcpy(currCmd[numOfcommands].taskDuration, pch);
                    }else { estimatesErrorMessage(line, lineCpy); }
                    
                    //If there is more content on the line then throw error as doesn't match format.
                     pch = strtok(NULL, " ");
                     if(pch != NULL){
...
{   //Checks that every command from the crontab-file also exists in the estimates-file.
    for(int eachCrontab = 0; eachCrontab < numOfCrontabCommands; ++eachCrontab)
    {
        ...
        //If valid still = 0 then current command didn't appear in estimates, and should throw error.
        if(valid == 0)
        {
        ...
        do
        {
            while(fgets(mystring, MAX_COMMAND_LEN, fp) != NULL)
            {
                ...
                else
                {
                    ...
                    //Checks for both types of input: A number (0-59) and '*'
                    pch = strtok(mystring,delimiters);
                    digCheck = strtol(pch, &ptr, 10);
                    ...
                    //Checks for both types of input: A number (0-23) and '*'
                    ...
                    //Checks for both types of input: number (1-31) and '*'
                    pch = strtok(NULL, delimiters);
                    digCheck = strtol(pch, &ptr, 10);
                    ...
                    //Checks for all 3 types of input: 3-char input that's lowercase, number (0-11) and '*'
                    pch = strtok(NULL, delimiters);
                    digCheck = strtol(pch, &ptr, 10);
                    if(pch != NULL && ((
                                        //Either a valid month number
                                       (digCheck >= 0 && digCheck <= 11 && *ptr == '\0') ||
                                        //Or a valid 3-char month input and lowercase
                                       ...
                                       isLowerCase(pch) !=0))
                                       //Or has '*' value and is just a single character.
                                       || (strchr(pch, '*') != NULL && strcmp(pch,"*") == 0)))
                    {
                    ...
                    //Checks for all 3 types of input: 3-char input that's lowercase, number (0-6) and '*'
                    pch = strtok(NULL,delimiters);
                    digCheck = strtol(pch, &ptr, 10);
                    if(pch != NULL && ((
                                        //A valid month number check
                                        (digCheck >= 0 && digCheck <= 6 && *ptr == '\0') ||
                                        //OR a valid 3-char day input and lowercase
                                        (((strcmp(pch,"mon") == 0) || (strcmp(pch,"tue") == 0) ||
                                        (strcmp(pch,"wed") == 0) || (strcmp(pch,"thu") == 0) ||
                                        (strcmp(pch,"fri") == 0) || (strcmp(pch,"sat") == 0) ||
                                        (strcmp(pch,"sun") == 0)) && isLowerCase(pch) !=0)   ||
                                        //OR has '*' value and is just a single character.
                                        (strchr(pch, '*') != NULL && strcmp(pch,"*") == 0))))
                    {
...
void simulator(int month)
{
    ...
    while(day_of_month <= days_in_month){
        ...
        while(currHour < 24)
        {
            //Goes through each minute
            for(int minute = 0; minute < 60; minute++)
            {
                //Goes through every command and checks if it should run, terminate, or do nothing.
                for(int i = 0; i < numOfCrontabCommands; ++i)
                {
                    //If the command is supposed to execute in the provided month, do more checks.
                            ...
                            for(int j = 0; j < numOfCrontabCommands; ++j)
                            {
                                ...
                                if(strcmp(currCmd[j].commandName,cronCmd[i].commandName) == 0)
                                {
                                    ...
                                    if((
                                      ...
                                      (atoi(cronCmd[i].hour) == currHour || (strchr(cronCmd[i].hour, '*') != NULL)) &&
                                      (atoi(cronCmd[i].minute) == minute || (strchr(cronCmd[i].minute, '*') != NULL)) && running < 20)
                                    {
                                        ++running;
                                        ...
                                        if(atoi(cronCmd[i].hour)*60 != 0 && atol(cronCmd[i].minute) != 0){
                                        ...
                                        {
                                            initialTime = currHour*60 + minute;
                                            finishTime = initialTime + atol(currCmd[j].taskDuration);
                                            finishHour = (finishTime / 60);
                                        }
                                        
                                        //Goes through the runningCommands array to find an 'empty' location to store the newly running commands finish time.
                                        for(int z = 0; z < 25; ++z){
                                            if(strlen(runningCommands[z].commandName) == 0) //Found empty location in array
                                            {
